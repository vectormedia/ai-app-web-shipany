...
import { ExtendedSignInForm } from './extended-sign-in-form';

export function ExtendedSignModal({
  callbackUrl = '/',
}: {
  callbackUrl?: string;
}) {
    ...
    return (
      <Dialog open={isShowSignModal} onOpenChange={setIsShowSignModal}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <ExtendedSignInForm callbackUrl={callbackUrl} />
        </DialogContent>
      </Dialog>
    );
  }
  ...
  return (
    <Drawer open={isShowSignModal} onOpenChange={setIsShowSignModal}>
      <DrawerContent>
        ...
        <ExtendedSignInForm callbackUrl={callbackUrl} className="mt-8 px-4" />
